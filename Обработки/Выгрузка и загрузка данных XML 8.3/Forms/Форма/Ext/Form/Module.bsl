...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка);
...
Процедура ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)	
	...
	Объект.ОрганизацияНовая = Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор("f3cb7aa2-b7e5-11e7-815a-005056827059"));
	Объект.ОрганизацияСтарая = Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор("95abc896-b12f-11e8-a227-509a4caf7e09"));
	Объект.ОрганизацияЭталон = Справочники.Организации.НайтиПоНаименованию("АО ""СВОБОДА"" Новая Эталон");
	Объект.ИспользоватьПодменуОрганизаций = Истина;
		
	// Шаблоны
	ГруппаШаблоны = Элементы.Добавить("ОтметитьПоШаблону", 
		Тип("ГруппаФормы"), 
		Элементы.ДеревоМетаданных.КоманднаяПанель);
		
	ГруппаШаблоны.Вид = ВидГруппыФормы.Подменю;
	
	ДобавитьКомандуШаблон(ГруппаШаблоны,"КонстантыОрганизацииУчетнаяПолитика");
	ДобавитьКомандуШаблон(ГруппаШаблоны,"ВыгрузкаНСИ");
	ДобавитьКомандуШаблон(ГруппаШаблоны,"ВводОстатков");
	ДобавитьКомандуШаблон(ГруппаШаблоны,"СкладИЛогистика");
	
	// Подмена организаций
	ГруппаПодменаОрганизаций = Элементы.Добавить("ПодменаОрганизаций", 
		Тип("ГруппаФормы"), 
		Элементы.ГруппаЗагрузка);
	ГруппаПодменаОрганизаций.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	
	Элемент = Элементы.Добавить("ИспользоватьПодменуОрганизаций",
		Тип("ПолеФормы"), 
		ГруппаПодменаОрганизаций);
	Элемент.ПутьКДанным = "Объект.ИспользоватьПодменуОрганизаций";
	Элемент.Вид = ВидПоляФормы.ПолеФлажка;
	
	ДобавитьЭлементОрганизация(ГруппаПодменаОрганизаций,"ОрганизацияНовая");
	ДобавитьЭлементОрганизация(ГруппаПодменаОрганизаций,"ОрганизацияСтарая");
	ДобавитьЭлементОрганизация(ГруппаПодменаОрганизаций,"ОрганизацияЭталон");
	
КонецПроцедуры

&НаКлиенте
Процедура ПриОткрытии(Отказ)
	ОтобразитьИдентификаторыОрганизаций();
КонецПроцедуры
...
&НаСервере
Процедура ДобавитьКомандуШаблон(Родитель, ИмяКоманды)	
	
	НоваяКоманда = Команды.Добавить(ИмяКоманды);
	НоваяКоманда.Действие = "Подключаемый_"+ИмяКоманды;
...
&НаСервере
Процедура Шаблон_ВыгрузкаНСИНаСервере()

	Дерево = РеквизитФормыВЗначение("Объект").ДеревоМетаданных;
	
	Корень = Дерево.Строки[0];	
	Корень.Выгружать = 2;
	Корень.ВыгружатьПриНеобходимости = 2;
	
	ВеткаСправочники = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Справочники"))[0];	
	ВеткаСправочники.Выгружать = 2;
	
	НеВыгружать = Новый Массив;
	НеВыгружать.Добавить("Назначения");
	НеВыгружать.Добавить("КлючиАналитикиУчетаНаборов");
	НеВыгружать.Добавить("КлючиАналитикиУчетаПартийПроизводства");
	НеВыгружать.Добавить("КлючиАналитикиУчетаНоменклатуры");
	НеВыгружать.Добавить("КлючиАналитикиУчетаПоПартнерам");
	НеВыгружать.Добавить("КлючиАналитикиУчетаПартий");
	// Органзации неоходимо прогрузить предварительно, повторно выгружать не будем
	НеВыгружать.Добавить("Организации"); 
	ВеткаОрганизации = ВеткаСправочники.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Организации"))[0];	
	ВеткаОрганизации.ВыгружатьПриНеобходимости = 0;
	ВеткаСправочники.ВыгружатьПриНеобходимости = 2;

	Для Каждого ЭлементДерева Из ВеткаСправочники.Строки Цикл
			
		Если НеВыгружать.Найти(ЭлементДерева.ПолноеИмяМетаданных) <> Неопределено Тогда
			Продолжить;	
		КонецЕсли;
		
		ЭлементДерева.Выгружать = 1;
		
	КонецЦикла;
	
	// ПВХ
	ИзменитьПравилаВыгрузки("ПланыВидовХарактеристик",Истина,Истина,Корень);
...
&НаСервере
Процедура Шаблон_КонстантыОрганизацииУчетнаяПолитикаНаСервере()
	...
	// Константы
	ВеткаКонстанты = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Константы"))[0];	
	ВеткаКонстанты.Выгружать = 1;
	
	Для Каждого ЭлементДерева Из ВеткаКонстанты.Строки Цикл	
		ЭлементДерева.Выгружать = 1;	
	КонецЦикла;
	...
	ВеткаОрганизации = ВеткаСправочники.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Организации"))[0];	
	ВеткаОрганизации.Выгружать = 1;

	// УчетнаяПоличтика
	ВеткаРегистры = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","РегистрыСведений"))[0];
	ВеткаРегистры.Выгружать = 2;
	
	ВеткаУчетнаяПоличтика = ВеткаРегистры.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","УчетнаяПолитикаОрганизаций"))[0];
	ВеткаУчетнаяПоличтика.Выгружать = 1;
...
&НаСервере
Процедура Шаблон_СкладИЛогистика()
	...
	ОчиститьДочерниеОбъекты(Корень);
	
	ИзменитьПравилаВыгрузки("Документы", 2, 2, Корень);
		
	ВеткаДокументы = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Документы"))[0];

	
	
	МассивДокументовДляВыгрузки = Новый Массив;
	МассивДокументовДляВыгрузки.Добавить("ЗаказНаПеремещение");
	МассивДокументовДляВыгрузки.Добавить("ПеремещениеТоваров");
	МассивДокументовДляВыгрузки.Добавить("ЗаказНаСборку");
	МассивДокументовДляВыгрузки.Добавить("СборкаТоваров");	
	МассивДокументовДляВыгрузки.Добавить("ПриходныйОрдерНаТовары");
	МассивДокументовДляВыгрузки.Добавить("РасходныйОрдерНаТовары");
	МассивДокументовДляВыгрузки.Добавить("ЗаказНаВнутреннееПотребление");
	МассивДокументовДляВыгрузки.Добавить("ВнутреннееПотреблениеТоваров");
	МассивДокументовДляВыгрузки.Добавить("ОприходованиеИзлишковТоваров");
	МассивДокументовДляВыгрузки.Добавить("СписаниеНедостачТоваров");
	МассивДокументовДляВыгрузки.Добавить("ПересортицаТоваров");
	МассивДокументовДляВыгрузки.Добавить("ОрдерНаОтражениеИзлишковТоваров");
	МассивДокументовДляВыгрузки.Добавить("ОрдерНаОтражениеНедостачТоваров");
	МассивДокументовДляВыгрузки.Добавить("ОрдерНаОтражениеПересортицыТоваров");
	
	ИзменитьПравилаВыгрузкиДляСписка(МассивДокументовДляВыгрузки, 1, 0, ВеткаДокументы);
	
	
	
	ИзменитьПравилаВыгрузки("ЗаказНаПеремещение", 1, 1, ВеткаДокументы);
	ИзменитьПравилаВыгрузки("ЗаказНаСборку", 1, 1, ВеткаДокументы);
	ИзменитьПравилаВыгрузки("ЗаказНаВнутреннееПотребление", 1, 1, ВеткаДокументы);

	
	
	ЗначениеВРеквизитФормы(Дерево,"Объект.ДеревоМетаданных");

	Объект.ВыгружатьСДокументомЕгоДвижения = Ложь;
	
КонецПроцедуры

&НаСервере
Процедура Шаблон_ВводОстатковНаСервере()
	...
	ВеткаСправочники = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Справочники"))[0];	
	ВеткаДокументы = Корень.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных","Документы"))[0];	
	ВеткаДокументы.Выгружать = 2;
	
	МассивОбъектов = Новый Массив;
	МассивОбъектов.Добавить("ВводОстатков");
	МассивОбъектов.Добавить("ВводОстатковВнеоборотныхАктивов2_4");
	...
	ИзменитьПравилаВыгрузкиДляСписка(МассивОбъектов,1,0,ВеткаДокументы);
...
&НаСервере
Процедура ИзменитьПравилаВыгрузки(ИмяОбъекта, Выгружать, ВыгружатьПриНеобходимости, ЭлементРодитель, УстанавливатьДляДочерних = Истина)
	
	Ветка = ЭлементРодитель.Строки.НайтиСтроки(Новый Структура("ПолноеИмяМетаданных", ИмяОбъекта))[0];
	Ветка.Выгружать = Выгружать;
	Ветка.ВыгружатьПриНеобходимости = ВыгружатьПриНеобходимости;
	
	Если УстанавливатьДляДочерних Тогда
		Для каждого Элемент из Ветка.Строки Цикл
			
			Если Выгружать = 0 ИЛИ Выгружать = 1 Тогда
				Элемент.Выгружать = Выгружать;
			КонецЕсли;
			
			Если ВыгружатьПриНеобходимости = 0 ИЛИ ВыгружатьПриНеобходимости = 1 Тогда
				Элемент.ВыгружатьПриНеобходимости = ВыгружатьПриНеобходимости;
			КонецЕсли;
...
&НаСервере
Процедура ИзменитьПравилаВыгрузкиДляСписка(СписокОбъектов, Выгружать, ВыгружатьПриНеобходимости, ЭлементРодитель)
	
	Для каждого Элемент из СписокОбъектов Цикл
		ИзменитьПравилаВыгрузки(Элемент, Выгружать, ВыгружатьПриНеобходимости, ЭлементРодитель);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ОчиститьДочерниеОбъекты(ЭлементРодитель)
	
	Для каждого Элемент из ЭлементРодитель.Строки Цикл
		Элемент.Выгружать = 0;
		Элемент.ВыгружатьПриНеобходимости = 0;
		Для каждого Элемент2 из Элемент.Строки Цикл
			Элемент2.Выгружать = 0;
			Элемент2.ВыгружатьПриНеобходимости = 0;
		КонецЦикла;
	КонецЦикла;
...
&НаКлиенте
Процедура Подключаемый_ВыгрузкаНСИ(Команда) Экспорт
	
	Шаблон_ВыгрузкаНСИНаСервере();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_КонстантыОрганизацииУчетнаяПолитика(Команда) Экспорт
	
	Шаблон_КонстантыОрганизацииУчетнаяПолитикаНаСервере();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СкладИЛогистика(Команда) Экспорт
	
	Шаблон_СкладИЛогистика();

КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ВводОстатков(Команда) Экспорт
	
	Шаблон_ВводОстатковНаСервере();

КонецПроцедуры

#КонецОбласти

#Область ПодменаОрганизаций

&НаСервере
Процедура ДобавитьЭлементОрганизация(Родитель, Имя)	
	...
	Элемент.УстановитьДействие("ПриИзменении","ОрганизацияПриИзменении"); 
	
	Элемент = Элементы.Добавить(Имя+"ИД", 
		Тип("ДекорацияФормы"), 
		Родитель);
...
&НаКлиенте
Процедура ОтобразитьИдентификаторыОрганизаций()
	
	Элементы.ОрганизацияНоваяИД.Заголовок = Объект.ОрганизацияНовая.УникальныйИдентификатор();
	Элементы.ОрганизацияСтараяИД.Заголовок = Объект.ОрганизацияСтарая.УникальныйИдентификатор();
	Элементы.ОрганизацияЭталонИД.Заголовок = Объект.ОрганизацияЭталон.УникальныйИдентификатор();
