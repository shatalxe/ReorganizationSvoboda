...
Процедура Шаблон_ВводОстатковНаСервере()
	...
	Выгружать = Новый Массив;
	Выгружать.Добавить("ВводОстатков");
	Выгружать.Добавить("ВводОстатковВнеоборотныхАктивов2_4");
	...
	Для Каждого ЭлементДерева Из ВеткаСправочники.Строки Цикл
		...
		Если Выгружать.Найти(ЭлементДерева.ПолноеИмяМетаданных) = Неопределено Тогда
			Продолжить;	
		КонецЕсли;
